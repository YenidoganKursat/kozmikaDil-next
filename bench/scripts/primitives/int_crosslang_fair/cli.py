"""CLI for fair integer cross-language runtime benchmark."""
...
from .common import BITS, INT_PRIMITIVES, OPS, RESULT_DIR
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Fair integer runtime benchmark across languages")
    ...
    parser.add_argument("--out-name", default="int_crosslang_fair_runtime.json")
    return parser
    ...
    if not args.skip_cross_lang:
        ...
        for language, runner in cross_runners:
            ...
            for op in OPS:
                ...
                op_rows[op] = row
                print(
                    f"{language:<7} i64  {op} {row.ns_op:10.3f} ns/op checksum={row.checksum}",
                    flush=True,
                )
            if available:
                cross[language] = _rows_to_dict({"i64": op_rows})["i64"]
            else:
                print(f"{language:<7} unavailable on this machine (skipped)", flush=True)

    payload = {
        "method": "fair total runtime over full loop, deterministic integer stream, median ns/op",
        "loops": args.loops,
        "lanes": args.lanes,
        "pow_profile": args.pow_profile,
        "runs": args.runs,
        ...
        "interpret_path": args.interpret_path,
        "ops": OPS,
        "kozmika_i_series": kozmika_by_mode,
        "cross_lang_i64": cross,
    }
